...
export default function ModalLivro({ id, fecharModal }) {
  const [apenasLivro, setLivros] = useState({});

  useEffect(() => {
    console.log('ID recebido no modal:', id);
    if (id) {
      LivroService.UmLivro(id)
        .then((livro) => {
  ...
  const AlugarLivro = async () => {
    try {
      ...
      fecharModal(); // Corrigido para fechar corretamente o modal
    } catch (error) {
      ...
      <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        <div className="modal-content">
          ...
          <div className="modal-body">
            {apenasLivro && apenasLivro.titulo ? (
              <div className="livro-detalhes">
                ...
                <div className="txt-modal">
                  ...
                  <p>Sinopse: {apenasLivro.descricao}</p>
                </div>
              </div>
            ) : (
              <p>Carregando...</p>
